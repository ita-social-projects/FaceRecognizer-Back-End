...
std::string SQLServer::ExePath()
{
	...
	std::wstring::size_type pos = std::wstring(
            reinterpret_cast<const wchar_t *const>(buffer)).find_last_of(L"\\/");
	std::wstring wpath = std::wstring(
            reinterpret_cast<const wchar_t *const>(buffer)).substr(0, pos);
